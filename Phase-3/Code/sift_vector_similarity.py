...
def getSimilarityNew1(c1,c2, simMeasure):
    if simMeasure == 1:
    ...
    for i in c1:
        ...
        for j in c2:
            if (len(i) == 0) or (len(j) == 0):
               ...
               sim = similarityFunction(i, j)
            
            dis.append(sim)
        dis.sort()
        if len(dis)>1 and dis[1]>=1.5*dis[0]:
...
def getSimilarityNew(c1,c2, simMeasure):
    ...
    for i in range(0,len(c1)):
        dis = list()
        if i<len(c2):
           sim = spatial.distance.euclidean(c1[i],c2[i])
           total += sim
           count += 1
           if sim > max_dis:
              max_dis = sim
    if count == 0:
       count = 1
    similarity = float(total)/count
    if max_dis == 0:
       max_dis = 1
    similarity = (float)(similarity/max_dis)
...
def computeFrameSimilarity(f1,f2,simMeasure):
    noOfCells = len(f1)
    similarityOfFrame = 0
    noOfCellsToSkip = 0
    for k in xrange(noOfCells):
        if (len(f1[k]) == 0) or (len(f2[k]) == 0):
           noOfCellsToSkip += 1
        else: 
           similarityOfCell = (getSimilarityNew(f1[k], f2[k], simMeasure))
           similarityOfFrame += similarityOfCell
    if(noOfCells <= noOfCellsToSkip):
        return 0.0
    return similarityOfFrame/(noOfCells-noOfCellsToSkip)

def computeFrameSimilarityEuclidean(f1,f2):
    return computeFrameSimilarity(f1,f2,2)

def computeFrameSimilarityCosine(f1,f2):
    computeFrameSimilarity(f1,f2,1)
...
def main():
    ...
    #print computeFrameSimilarity(f1,f2,2)
