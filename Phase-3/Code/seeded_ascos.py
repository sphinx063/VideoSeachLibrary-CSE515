...
def ascos_similarity(graph, seed_frame_input):
    ...
    for i in range(n):
        seed_node = node_ids[i]
        if seed_node in seed_frames:
           ...
           for j in range(n):
               if i != j:
                  ...
                  if weight != 0:
                     ...
                     if node_ids[i] not in seed_frames:
                         sim[i, j] = (float) (c * sim_ij / (weight * n))
...
seed_frame_input = raw_input()

ascos_similarity(graph, seed_frame_input)
